...
scheduler.xy.map_date_width = 188, scheduler.xy.map_description_width = 400, scheduler.config.map_resolve_event_location = !0, scheduler.config.map_resolve_user_location = !0, scheduler.config.map_initial_position = new google.maps.LatLng(48.724, 8.215), scheduler.config.map_error_position = new google.maps.LatLng(15, 15), scheduler.config.map_infowindow_max_width = 300, scheduler.config.map_type = google.maps.MapTypeId.ROADMAP, scheduler.config.map_zoom_after_resolve = 15, scheduler.locale.labels.marker_geo_success = "It seems you are here.",
    scheduler.locale.labels.marker_geo_fail = "Sorry, could not get your current position using geolocation.", scheduler.templates.marker_date = scheduler.date.date_to_str("%Y-%m-%d %H:%i"), scheduler.templates.marker_text = function(e, t, a) {
        ...
        function a() {
            ...
            for (var t = scheduler._waiAria.mapAttrString(), a = "<div " + t + " class='dhx_map_area'>", r = 0; r < e.length; r++) {
                var i = e[r],
                    n = i.id == scheduler._selected_event_id ? "dhx_map_line highlight" : "dhx_map_line",
                    s = i.color ? "background:" + i.color + ";" : "",
                    d = i.textColor ? "color:" + i.textColor + ";" : "",
                    t = scheduler._waiAria.mapRowAttrString(i),
                    l = scheduler._waiAria.mapDetailsBtnString();
                a += "<div " + t + " class='" + n + "' event_id='" + i.id + "' style='" + s + d + (i._text_style || "") + " width: " + (scheduler.xy.map_date_width + scheduler.xy.map_description_width + 2) + "px;'><div style='width: " + scheduler.xy.map_date_width + "px;' >" + scheduler.templates.map_time(i.start_date, i.end_date, i) + "</div>",
                    a += "<div " + l + " class='dhx_event_icon icon_details'>&nbsp</div>", a += "<div class='line_description' style='width:" + (scheduler.xy.map_description_width - 25) + "px;'>" + scheduler.templates.map_text(i.start_date, i.end_date, i) + "</div></div>"
            }
            a += "<div class='dhx_v_border' style='left: " + (scheduler.xy.map_date_width - 2) + "px;'></div><div class='dhx_v_border_description'></div></div>", scheduler._els.dhx_cal_data[0].scrollTop = 0, scheduler._els.dhx_cal_data[0].innerHTML = a, scheduler._els.dhx_cal_data[0].style.width = scheduler.xy.map_date_width + scheduler.xy.map_description_width + 1 + "px";
            var o = scheduler._els.dhx_cal_data[0].firstChild.childNodes;
            ...
            for (var r = 0; r < o.length - 2; r++) scheduler._rendered[r] = o[r]
        }
        ...
        i.className = "dhx_map", i.id = "dhx_gmap", i.style.dispay = "none";
        var n = scheduler._obj;
        n.appendChild(i), scheduler._els.dhx_gmap = [], scheduler._els.dhx_gmap.push(i), r("dhx_gmap");
        var s = {
            ...
            d = new google.maps.Map(document.getElementById("dhx_gmap"), s);
        ...
        scheduler.config.map_infowindow_max_width && (l.maxWidth = scheduler.config.map_infowindow_max_width), scheduler.map = {
            _points: [],
            _markers: [],
            _infowindow: new google.maps.InfoWindow(l),
            ...
            i.style.zIndex = "5", d.setZoom(d.getZoom())
        }), google.maps.event.addListener(d, "tilesloaded", function(e) {
            i.style.zIndex = "5"
        }), i.style.display = "none", scheduler.attachEvent("onSchedulerResize", function() {
        ...
        scheduler.render_data = function(e, t) {
            ...
            a();
            for (var r = scheduler.get_visible_events(), i = 0; i < r.length; i++) scheduler.map._markers[r[i].id] || c(r[i], !1, !1)
        }, scheduler.map_view = function(i) {
            scheduler.map._initialization_count++;
            var n, s = scheduler._els.dhx_gmap[0];
            if (scheduler._els.dhx_cal_data[0].style.width = scheduler.xy.map_date_width + scheduler.xy.map_description_width + 1 + "px", scheduler._min_date = scheduler.config.map_start || scheduler._currentDate(), scheduler._max_date = scheduler.config.map_end || scheduler.date.add(scheduler._currentDate(), 1, "year"), scheduler._table_view = !0, e(i), i) {
                t(), a(), s.style.display = "block", r("dhx_gmap"), n = scheduler.map._obj.getCenter();
                for (var d = scheduler.get_visible_events(), l = 0; l < d.length; l++) scheduler.map._markers[d[l].id] || c(d[l]);
            } else s.style.display = "none";
            google.maps.event.trigger(scheduler.map._obj, "resize"), 0 === scheduler.map._initialization_count && n && scheduler.map._obj.setCenter(n), scheduler._selected_event_id && _(scheduler._selected_event_id)
        };
        var _ = function(e) {
            ...
            c = function(e, t, a) {
                ...
                var i = scheduler.templates.marker_text(e.start_date, e.end_date, e);
                scheduler._new_event || (scheduler.map._infowindows_content[e.id] = i, scheduler.map._markers[e.id] && scheduler.map._markers[e.id].setMap(null), scheduler.map._markers[e.id] = new google.maps.Marker({
                    position: r,
        ...
        scheduler.attachEvent("onEventChanged", function(e, t) {
            if (this._latLngUpdate) this._latLngUpdate = !1;
                ...
                a.start_date < scheduler._min_date && a.end_date > scheduler._min_date || a.start_date < scheduler._max_date && a.end_date > scheduler._max_date || a.start_date.valueOf() >= scheduler._min_date && a.end_date.valueOf() <= scheduler._max_date ? (scheduler.map._markers[e] && scheduler.map._markers[e].setMap(null), h(a)) : (scheduler._selected_event_id = null, scheduler.map._infowindow.close(), scheduler.map._markers[e] && scheduler.map._markers[e].setMap(null))
            }
            return !0
            ...
            return scheduler._dataprocessor || (t.start_date < scheduler._min_date && t.end_date > scheduler._min_date || t.start_date < scheduler._max_date && t.end_date > scheduler._max_date || t.start_date.valueOf() >= scheduler._min_date && t.end_date.valueOf() <= scheduler._max_date) && (scheduler.map._markers[e] && scheduler.map._markers[e].setMap(null), h(t)), !0
