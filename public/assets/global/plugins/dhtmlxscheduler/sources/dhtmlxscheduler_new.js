...
dhtmlDragAndDropObject.prototype.preCreateDragCopy = function(e) {
    if ((e || window.event) && (e || event).button == 2) return;
...
(function() {
    function isVisible(node) {
        ...
        var recurringSection = scheduler._locate_css({
            target: node
...
dataProcessor.prototype = {
        ...
        if (this.callEvent("onRowMark", [id, state, mode, invalid])) {
            ...
            if (invalid && invalid.details) {
                ...
                for (var i = 0; i < invalid.details.length; i++)
                    if (invalid.details[i]) this.obj[this._methods[1]](id, i, str);
            }
        }
    },
    ...
    serialize: function(data, id) {
        ...
        if (typeof id != "undefined") return this.serialize_one(data, "");
        else {
            var stack = [];
            var keys = [];
            for (var key in data)
    ...
    _applyPayload: function(url) {
        ...
        if (this._payload)
            for (var key in this._payload) url = url + ajax.urlSeparator(url) + this.escape(key) + "=" + this.escape(this._payload[key]);
        return url;
    },
    _sendData: function(a1, rowId) {
        ...
        var back = function(xml) {
            ...
            if (rowId) ids.push(rowId);
                ...
                for (var key in a1) ids.push(key);
    ...
    afterUpdateCallback: function(sid, tid, action, btag) {
        ...
        if ((this._uActions) && (this._uActions[action]) && (!this._uActions[action](btag))) return (delete this._in_progress[marker]);
    ...
    cleanUpdate: function(id) {
        if (id)
            for (var i = 0; i < id.length; i++) delete this._in_progress[id[i]];
    },
    finalizeUpdate: function() {
        if (this._waitMode) this._waitMode--;
        if ((this.obj.mytype == "tree" || this.obj._h2) && this.updatedRows.length) this.sendData();
...
(function() {
    ...
    function stripHTMLLite(htmlText) {
        return (htmlText + "").replace(htmlTags, " ").
        replace(extraSpaces, " ");
    }
...
scheduler._init_once = function() {
    ...
    dhtmlxEvent(window, "resize", function() {
        if (!isAttachedNode(scheduler._obj)) return;
...
scheduler.unset_actions = function() {
    for (var a in this._els)
        if (this._click[a])
            for (var i = 0; i < this._els[a].length; i++) this._els[a][i].onclick = null;
...
scheduler.set_actions = function() {
    for (var a in this._els)
        if (this._click[a])
            for (var i = 0; i < this._els[a].length; i++) this._els[a][i].onclick = scheduler._click[a];
    this._obj.onselectstart = function(e) {
        return false;
    };
    this._obj.onmousemove = function(e) {
        if (!scheduler._temp_touch_block) scheduler._on_mouse_move(e || event);
    };
    this._obj.onmousedown = function(e) {
        if (!scheduler._ignore_next_click) scheduler._on_mouse_down(e || event);
    };
    this._obj.onmouseup = function(e) {
        if (!scheduler._ignore_next_click) scheduler._on_mouse_up(e || event);
    };
...
scheduler._on_dbl_click = function(e, src) {
    ...
    switch (name) {
        ...
        case "dhx_cal_event_clear":
            ...
            if (this.config.details_on_dblclick || this._table_view || !this.getEvent(id)._timed || !this.config.select) this.showLightbox(id);
            else this.edit(id);
            break;
        case "dhx_time_block":
...
scheduler._mouse_coords = function(ev) {
    ...
    if (!_isIE && (ev.pageX || ev.pageY)) pos = {
        x: ev.pageX,
        y: ev.pageY
    };
    else pos = {
    ...
    if (handler) {
        ...
        if (!this._table_view) {
            ...
            if (scheduler._drag_mode || this._mode == "month") pos.y = (Math.max(0, Math.ceil(column) - 1) + Math.max(0, dy - 1) * 7) * 24 * 60 / this.config.time_step;
    ...
    if (this._drag_mode && this._drag_id) {
        ...
        if (moved && (this._drag_event._dhx_changed || !this._drag_event.start_date || ev.start_date.valueOf() != this._drag_event.start_date.valueOf() || ev.end_date.valueOf() != this._drag_event.end_date.valueOf())) {
            ...
            if (!this.callEvent("onBeforeEventChanged", [ev, e, is_new, this._drag_event])) {
                if (is_new) this.deleteEvent(ev.id, true);
                else {
...
scheduler._render_month_scale = function(div, dd /*month start*/ , sd /*view start*/ , rows) {
    ...
    for (var i = 0; i < rows; i++) {
        ...
        for (var j = 0; j < 7; j++) {
            ...
            var cls = "";
            if (sd < dd) cls = 'dhx_before';
            else if (sd >= ed) cls = 'dhx_after';
            else if (sd.valueOf() == cd.valueOf()) cls = 'dhx_now';
...
scheduler._get_fictional_event_length = function(end_date, ev_length, obj, back) {
    ...
    if (obj._start_correction || obj._end_correction) {
        var today;
        if (back) today = (sd.getHours() * 60 + sd.getMinutes()) - (obj.first_hour || 0) * 60;
        else today = (obj.last_hour || 0) * 60 - (sd.getHours() * 60 + sd.getMinutes());
...
scheduler.$ajax = {
    ...
    query: function(config) {
        this._call(
            (config.method || "GET"), config.url, config.data || "",
            (config.async || true), config.callback, null, config.headers);
    },
...
scheduler.date = {
    init: function() {
        ...
        for (var i = 0; i < s.length; i++) t[s[i]] = i;
...
scheduler.for_rendered = function(id, method) {
    for (var i = this._rendered.length - 1; i >= 0; i--)
        if (this._rendered[i].getAttribute("event_id") == id) method(this._rendered[i], i);
};
...
scheduler.event_updated = function(ev, force) {
    if (this.is_visible_events(ev)) this.render_view_data();
...
scheduler.isOneDayEvent = function(ev) {
    ...
    if (!delta) return ev.start_date.getMonth() == ev.end_date.getMonth() && ev.start_date.getFullYear() == ev.end_date.getFullYear();
    else {
    ...
    for (var id in this._events)
        if (this.is_visible_events(this._events[id]))
            if (!only_timed || this._events[id]._timed)
                if (this.filter_event(id, this._events[id])) stack.push(this._events[id]);
    return stack;
};
...
scheduler.render_view_data = function(evs, hold) {
    ...
    if (this.config.multi_day && !this._table_view) {
        ...
        for (var i = 0; i < evs.length; i++) {
            if (this._is_main_area_event(evs[i])) tvs.push(evs[i]);
            else tvd.push(evs[i]);
        }
        // multiday events
...
scheduler._recalculate_timed = function(id) {
    if (!id) return;
    var ev;
    if (typeof(id) != "object") ev = this._events[id];
    else ev = id;
    if (!ev) return;
...
scheduler._pre_render_events = function(evs, hold) {
    ...
    if (this._table_view) {
        if (hold) this._colsS.heights = h_old;
        else {
...
scheduler._pre_render_events_line = function(evs, hold) {
            ...
            if (!sorderSet) {
                if (stack.length) {
                    if (stack.length <= stack[stack.length - 1]._sorder) {
                        if (!stack[stack.length - 1]._sorder) ev._sorder = 0;
                        else
    ...
    if (!hold) {
        ...
        for (var i = 0; i < evs_originals.length; i++) evs_originals[i]._count = days[evs_originals[i]._sday].max_count;
    }
    return evs;
};
...
scheduler._get_dnd_order = function(order, ev_height, max_height) {
    ...
    if (!this._drag_event._orig_sorder) this._drag_event._orig_sorder = order;
    else order = this._drag_event._orig_sorder;
...
scheduler.render_event_bar = function(ev) {
    ...
    var color = (ev.textColor ? ("color:" + ev.textColor + ";") : "");
    var style_text = ["position:absolute", "top:" + y + "px", "left:" + x + "px", "width:" + (x2 - x - 15) + "px",
        color,
        bg_color,
        (ev._text_style || "")
    ].join(";");
...
scheduler.showEvent = function(id, mode) {
    ...
    if (scheduler.matrix && scheduler.matrix[mode]) {
        ...
        if (rendered_event) scheduler._els.dhx_cal_data[0].scrollTop = scheduler.$domHelpers.getAbsoluteTop(rendered_event) - scheduler.$domHelpers.getAbsoluteTop(scheduler._els.dhx_cal_data[0]) - 20;
    }
...
scheduler._load = function(url, from) {
    ...
    if (this._load_mode) {
        ...
        while (to < this._max_date) {
            ...
            if (this._loaded[lf(from)] && cache_line) from = this.date.add(from, 1, this._load_mode);
        ...
        scheduler.$ajax.get(url + "&from=" + lf(from) + "&to=" + lf(to), function(l) {
            scheduler.on_load(l);
        });
        while (from < to) {
    ...
    } else scheduler.$ajax.get(url, function(l) {
        scheduler.on_load(l);
    });
...
scheduler._init_date = function(date) {
    if (!date) return null;
    if (typeof date == "string") return scheduler.templates.xml_date(date);
...
scheduler.ical = {
    parse: function(str) {
        ...
        while ((match = event_r.exec(data)) !== null) {
            ...
            var param;
            var param_r = /[^\r\n]+[\r\n]+/g;
            while ((param = param_r.exec(match[1])) !== null) this.parse_param(param.toString(), e);
...
scheduler.formSection = function(name) {
    ...
    if (!scheduler._lightbox) scheduler.getLightbox();
...
scheduler.form_blocks = {
    ...
    select: {
        render: function(sns) {
            ...
            for (var i = 0; i < sns.options.length; i++) html += "<option value='" + sns.options[i].key + "'>" + sns.options[i].label + "</option>";
            html += "</select></div>";
            return html;
        },
            ...
            for (var p = 0; p < time_format.length; p++) {
                ...
                switch (time_option) {
                    case "%Y":
                        ...
                        for (var i = 0; i < 10; i++) options += "<option value='" + (year + i) + "'>" + (year + i) + "</option>";
                        break;
                    case "%m":
                        selectBoxClass = "dhx_lightbox_month_select";
                        sns._time_format_order[2] = p;
                        //month
                        for (var i = 0; i < 12; i++) options += "<option value='" + i + "'>" + this.locale.date.month_full[i] + "</option>";
                        break;
                    case "%d":
                        selectBoxClass = "dhx_lightbox_day_select";
                        sns._time_format_order[1] = p;
                        //days
                        for (var i = 1; i < 32; i++) options += "<option value='" + i + "'>" + i + "</option>";
                        break;
                    case "%H:%i":
...
scheduler._init_touch_events = function() {
    var mobile = this.config.touch && (((navigator.userAgent.indexOf("Mobile") != -1) || (navigator.userAgent.indexOf("iPad") != -1) || (navigator.userAgent.indexOf("Android") != -1) || (navigator.userAgent.indexOf("Touch") != -1)) && !window.MSStream);
    if (mobile) {
...
scheduler._update_global_tip = function(init) {
    ...
    if (toptip) {
        ...
        if (scheduler._drag_mode == "create" || scheduler._drag_mode == "new-size") toptip.innerHTML = (scheduler.locale.labels.drag_to_create || "Drag to create") + time;
        else toptip.innerHTML = (scheduler.locale.labels.drag_to_move || "Drag to move") + time;
    }
};
...
scheduler._dp_init = function(dp) {
    ...
    dp._objToJson = function(obj, data, prefix) {
        ...
        for (var a in obj) {
            ...
            if (obj[a] && obj[a].getUTCFullYear) //not very good, but will work
                ...
                if (obj[a] && typeof obj[a] == "object") dp._objToJson(obj[a], data, prefix + a + ".");
                else data[prefix + a] = obj[a];
            }
        }
        return data;
    };
    ...
    dp.attachEvent("deleteCallback", function(upd, id) {
        ...
        } else if (this.obj._add_rec_marker) this.obj._update_callback(upd, id);
    });
};
                    ...
                    if (this && this.getAttribute) {
                        if (!this.getAttribute("dhxscheduler")) {
                            for (var key in config)
                                if (key != "data") scheduler.config[key] = config[key];
                            ...
                            if (config.data) scheduler.parse(config.data);
