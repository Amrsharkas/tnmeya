...
scheduler._dp_init = function(dp) {
    dp._sendData = function(a1, rowId) {
        ...
        if (this.serverProcessor.$proxy) {
            ...
            var to_send = [];
            for (var i in a1) to_send.push({
                id: i,
                data: a1[i],
                operation: this.getState(i)
            });
        ...
        if (this._tMode != "POST") a2.loadXML(a3 + ((a3.indexOf("?") != -1) ? "&" : "?") + this.serialize(a1, rowId));
        else a2.loadXML(a3, true, this.serialize(a1, rowId));
        this._waitMode++;
    };
    dp._updatesToParams = function(items) {
        var stack = {};
        for (var i = 0; i < items.length; i++) stack[items[i].id] = items[i].data;
...
if (window.dataProcessor) dataProcessor.prototype.init = function(obj) {
    this.init_original(obj);
    obj._dataprocessor = this;
    this.setTransactionMode("POST", true);
    if (!this.serverProcessor.$proxy) this.serverProcessor += (this.serverProcessor.indexOf("?") != -1 ? "&" : "?") + "editing=true";
};