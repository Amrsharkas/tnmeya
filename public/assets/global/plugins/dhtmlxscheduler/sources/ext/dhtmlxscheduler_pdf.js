...
(function() {
    ...
    function xml_body_header() {
        ...
        if (scheduler._mode == "week_agenda") {
            ...
            if (scheduler.matrix && scheduler.matrix[scheduler._mode]) {
                ...
                while (yh.className.indexOf("dhx_scale_holder") == -1) yh = yh.previousSibling;
                yh = yh.childNodes;
                xml += "<y>";
                for (var i = 0; i < yh.length; i++) xml += "\n<row><![CDATA[" + clean_html(yh[i].innerHTML) + "]]></row>";
                xml += "</y>";
    ...
    function xml_month(yh) {
        ...
        for (var i = 0; i < r.length; i++) {
            var days = [];
            for (var j = 0; j < r[i].cells.length; j++) days.push(r[i].cells[j].firstChild.innerHTML);
    ...
    function xml_body(colors) {
        ...
        if (scheduler._mode == "agenda" || scheduler._mode == "map") {
            for (var i = 0; i < evs.length; i++) xml += "<event><head><![CDATA[" + clean_html(evs[i].childNodes[0].innerHTML) + "]]></head><body><![CDATA[" + clean_html(evs[i].childNodes[2].innerHTML) + "]]></body></event>";
        } else if (scheduler._mode == "week_agenda") {
            for (var i = 0; i < evs.length; i++) xml += "<event day='" + evs[i].parentNode.getAttribute("day") + "'><body>" + clean_html(evs[i].innerHTML) + "</body></event>";
        } else if (scheduler._mode == "year") {
