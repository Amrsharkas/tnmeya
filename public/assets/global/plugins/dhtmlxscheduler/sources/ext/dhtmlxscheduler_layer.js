...
scheduler.attachEvent("onTemplatesReady", function() {
    ...
    (function() {
        var _cloneObj = function(obj) {
            ...
            for (var key in obj) temp[key] = _cloneObj(obj[key]);
            return temp;
        };
    ...
    scheduler.toggleLayer = function(tlayer, visible) { // visible is optional
        ...
        if (typeof visible != 'undefined') layer.visible = !!visible;
    ...
    scheduler.getLayer = function(tlayer) { // either string with layer name or event with layer property
        ...
        if (typeof tlayer == 'object') layer_name = tlayer.layer;
        for (var i = 0; i < scheduler.layers.length; i++) {
    ...
    scheduler._t_render_view_data = function(events) { // helper
        if (this.config.multi_day && !this._table_view) {
            ...
            for (var k = 0; k < events.length; k++) {
                if (events[k]._timed) tvs.push(events[k]);
                else tvd.push(events[k]);
            }
    ...
    scheduler.render_event = function(ev) {
        ...
        if (this._select_id == ev.id) {
            ...
            var icons_str = "";
            for (var i = 0; i < icons.length; i++) icons_str += "<div class='dhx_menu_icon " + icons[i] + "' title='" + this.locale.labels[icons[i]] + "'></div>";
