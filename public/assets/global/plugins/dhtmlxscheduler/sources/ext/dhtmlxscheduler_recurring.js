...
scheduler.attachEvent("onEventChanged", function(id) {
    ...
    if (this._is_virtual_event(id)) {
        ...
        for (var k = 0; k < this._rendered.length; k++) {
            if (this._rendered[k].getAttribute('event_id') == id) isEventFound = true;
        }
...
scheduler.repeat_date = function(ev, stack, non_render, from, to, maxCount) {
    ...
    while (td < ev.start_date || scheduler._fix_daylight_saving_date(td, from, ev, td, new Date(td.valueOf() + ev.event_length * 1000)).valueOf() <= from.valueOf() || td.valueOf() + ev.event_length * 1000 <= from.valueOf()) td = this.date.add(td, 1, ev.rec_pattern);
