...
(function() {
    ...
    for (var i = 0; i < events.length; i++) scheduler.attachEvent(events[i], hiding_function);
})();
...
scheduler.templates.quick_info_date = function(start, end, ev) {
    if (scheduler.isOneDayEvent(ev)) return scheduler.templates.day_date(start, end, ev) + " " + scheduler.templates.event_header(start, end, ev);
    else return scheduler.templates.week_date(start, end, ev);
};
...
scheduler.hideQuickInfo = function(forced) {
    ...
    if (qi && qi.parentNode) {
        ...
        if (qi.style.right == "auto") qi.style.left = -width + "px";
        else qi.style.right = -width + "px";
        if (forced) qi.parentNode.removeChild(qi);
        this.callEvent("onAfterQuickInfo", [eventId]);
    }
};
...
scheduler._init_quick_info = function() {
    if (!this._quick_info_box) {
        ...
        var html = "<div class=\"dhx_cal_qi_title\" style=\"height:" + sizes.quick_info_title + "px\" " + ariaAttr + ">" + "<div class=\"dhx_cal_qi_tcontent\"></div><div  class=\"dhx_cal_qi_tdate\"></div>" + "</div>" + "<div class=\"dhx_cal_qi_content\"></div>";
        //buttons
