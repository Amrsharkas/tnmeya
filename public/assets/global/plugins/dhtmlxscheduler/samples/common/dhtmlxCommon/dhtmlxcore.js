...
dhtmlx.assert_method = function(obj, config) {
    for (var key in config) dhtmlx.assert_method_process(obj, key, config[key].descr, config[key].args, (config[key].min || 99), config[key].skip);
};
dhtmlx.assert_method_process = function(obj, name, descr, rules, min, skip) {
    var old = obj[name];
    if (!skip) obj[name] = function() {
        if (arguments.length != rules.length && arguments.length < min) dhtmlx.log("warn", "Incorrect count of parameters\n" + obj[name].describe() + "\n\nExpecting " + rules.length + " but have only " + arguments.length);
        else
            for (var i = 0; i < rules.length; i++)
                if (!rules[i][3] && !rules[i][2](arguments[i])) dhtmlx.log("warn", "Incorrect method call\n" + obj[name].describe() + "\n\nActual value of " + (i + 1) + " parameter: {" + (typeof arguments[i]) + "} " + arguments[i]);
...
dhtmlx.assert_event_call = function(obj, name, args) {
    if (obj._event_check) {
        if (!obj._event_check[name]) dhtmlx.log("warn", "Not expected event call :" + name);
        else if (dhtmlx.isNotDefined(args)) dhtmlx.log("warn", "Event without parameters :" + name);
        else if (obj._event_check_size[name] != args.length) dhtmlx.log("warn", "Incorrect event call, expected " + obj._event_check_size[name] + " parameter(s), but have " + args.length + " parameter(s), for " + name + " event");
    }
};
dhtmlx.assert_event_attach = function(obj, name) {
    if (obj._event_check && !obj._event_check[name]) dhtmlx.log("warn", "Unknown event name: " + name);
};
//register names of properties, which can be used in object's configuration
dhtmlx.assert_property = function(obj, evs) {
    if (!obj._settings_check) obj._settings_check = {};
...
dhtmlx.assert_settings = function(mode, value, coll) {
    ...
    if (coll) {
        ...
        for (var i = 0; i < coll[mode].length; i++) {
            ...
            if (typeof rule == "function") check = check || rule(value);
            else if (typeof rule == "object" && typeof rule[1] == "function") {
                check = check || rule[1](value);
                if (check && rule[2]) dhtmlx["assert_check"](value, rule[2]); //temporary fix , for sources generator
            }
            if (check) break;
        }
...
dhtmlx.assert_info = function(name, set) {
    ...
    for (var i = 0; i < ruleset.length; i++) {
        if (typeof rule == "string") descr = ruleset[i];
        else if (ruleset[i].describe) expected.push(ruleset[i].describe());
        else if (ruleset[i][1] && ruleset[i][1].describe) expected.push(ruleset[i][1].describe());
    }
...
if (dhtmlx.assert_enabled()) {
    ...
    dhtmlx.assert_rule_key = function(obj) {
        ...
        t.describe = function() {
            var opts = [];
            for (var key in obj) opts.push(key);
...
dhtmlx.extend = function(target, source) {
    for (var method in source) target[method] = source[method];
    //applying asserts
...
dhtmlx.proto_extend = function() {
    ...
    for (var i = origins.length - 1; i > 0; i--) {
        ...
        for (var key in origins[i]) {
            if (key == "_init") construct.push(origins[i][key]);
            else if (!compilation[key]) compilation[key] = origins[i][key];
        }
    };
    if (origins[0]._init) construct.push(origins[0]._init);
...
dhtmlx.event = function(node, event, handler, master) {
    ...
    if (master) handler = dhtmlx.bind(handler, master);
...
dhtmlx.EventSystem = {
    ...
    callEvent: function(type, params) {
        ...
        if (this._map[type] && !this._map[type].callEvent(type, params)) return_value = false;
        return return_value;
    },
...
if (navigator.userAgent.indexOf('Opera') != -1) dhtmlx._isOpera = true;
else {
...
(function() {
    ...
    for (var i = 0; i < options.names.length; i++) {
        while (p = options[options.names[i]].pop()) {
            if (typeof d.style[p] != 'undefined') dhtmlx.env[options.names[i]] = true;
        }
    }
})();
dhtmlx.env.transform_prefix = (function() {
    var prefix;
    if (dhtmlx._isOpera) prefix = '-o-';
    else {
        prefix = ''; // default option
        if (dhtmlx._isFF) prefix = '-moz-';
        if (dhtmlx._isWebKit) prefix = '-webkit-';
    }
    return prefix;
})();
...
dhtmlx.Destruction = {
    ...
    destructor: function() {
        ...
        if (this._html) document.body.appendChild(this._html); //need to attach, for IE's GC
        this._html = null;
        if (this._obj) {
            this._obj.innerHTML = "";
        ...
        if (this.render) this.render = function() {}; //need in case of delayed method calls (virtual render case)
    }
};
...
dhtmlx.event(window, "unload", function() {
    ...
    if (dhtmlx.destructors) {
        for (var i = 0; i < dhtmlx.destructors.length; i++) dhtmlx.destructors[i].destructor();
        dhtmlx.destructors = [];
    }
...
dhtmlx.ajax.prototype = {
    ...
    getXHR: function() {
        if (dhtmlx._isIE) return new ActiveXObject("Microsoft.xmlHTTP");
    ...
    */
    send: function(url, params, call) {
        ...
        if (typeof params == "object") {
            ...
            for (var a in params) {
                var value = params[a];
                if (value === null || value === dhtmlx.undefined) value = "";
        ...
        if (this.post) x.setRequestHeader('Content-type', 'application/x-www-form-urlencoded');
            ...
            if (!x.readyState || x.readyState == 4) {
                ...
                if (call && self)
                    for (var i = 0; i < call.length; i++) //there can be multiple callbacks
                        if (call[i]) call[i].call((self.master || self), x.responseText, x.responseXML, x);
                self.master = null;
...
dhtmlx.AtomDataLoader = {
    ...
    load: function(url, call) {
        ...
        if (typeof call == "string") { //second parameter can be a loading type or callback
            ...
            call = arguments[2];
        } else this.data.driver = dhtmlx.DataDriver[this._settings.datatype || "xml"];
        ...
        if (window.dhx4) {
            dhx4.ajax.get(url, dhtmlx.bind(function(x) {
                ...
                if (this._onLoad) this._onLoad.call(this, text, xml, loader);
                if (call) call.call(this, text, xml, loader);
            }, this));
        } else dhtmlx.ajax(url, [this._onLoad, call], this);
    },
        ...
        if (typeof data == "string") {
            ...
            for (var i = 0; i < temp.data.length; i++) {
                ...
                for (var j = 0; j < header.length; j++) {
                    if (typeof(temp.data[i][j]) != "undefined") item[header[j]] = temp.data[i][j];
                }
...
dhtmlx.DataDriver.html = {
    ...
    */
    ...
    //get array of records
    getRecords: function(data) {
        if (data.tagName) return data.childNodes;
        return data;
    },
    //get hash of properties for single record
    getDetails: function(data) {
...
dhtmlx.DataDriver.xml = {
        ...
        for (var i = 0; i < b.length; i++) {
            if (b[i].nodeType == 1) {
                ...
                if (typeof z[name] != "undefined") {
                    if (!(z[name] instanceof Array)) z[name] = [z[name]];
    ...
    nodeValue: function(node) {
        if (node.firstChild) return node.firstChild.wholeText || node.firstChild.data;
        return "";
    },
    ...
    checkResponse: function(text, xml) {
        if (xml && (xml.firstChild && xml.firstChild.tagName != "parsererror")) return xml;
...
dhtmlx.DataLoader = {
    ...
    load: function(url, call) {
        ...
        if (!this.data.feed) this.data.feed = function(from, count) {
            //allow only single request at same time
            if (this._load_count) return this._load_count = [from, count]; //save last ignored request
            else this._load_count = true;
            this.load(url + ((url.indexOf("?") == -1) ? "?" : "&") + "posStart=" + from + "&count=" + count, function() {
                //after loading check if we have some ignored requests
                var temp = this._load_count;
                this._load_count = false;
                if (typeof temp == "object") this.data.feed.apply(this, temp); //load last ignored request
            });
        };
    },
    //default after loading callback
...
dhtmlx.DataStore.prototype = {
    ...
    _parse: function(data) {
        ...
        if (info._key) dhtmlx.security_key = info._key;
        //get array of records
        ...
        for (var i = 0; i < recs.length; i++) {
            ...
            if (this.extraParser) this.extraParser(temp);
            if (this._scheme) {
                if (this._scheme.$init) this._scheme.$update(temp);
                else if (this._scheme.$update) this._scheme.$update(temp);
            }
        }
    ...
    update: function(id, data) {
        if (this._scheme && this._scheme.$update) this._scheme.$update(data);
    ...
    refresh: function(id) {
        ...
        if (id) this.callEvent("onStoreUpdated", [id, this.pull[id], "update"]);
    ...
    sync: function(source, filter, silent) {
        ...
        if (source.name != "DataStore") source = source.data;
        var sync_logic = dhtmlx.bind(function(id, data, mode) {
            if (mode != "update" || filter) id = null;
            if (!id) {
            ...
            if (dhtmlx.debug_bind) dhtmlx.log("[sync:request] " + this.debug_sync_master.name + "@" + this.debug_sync_master._settings.id + " <= " + this.debug_bind_master.name + "@" + this.debug_bind_master._settings.id);
    ...
    add: function(obj, index) {
        if (this._scheme) {
            obj = obj || {};
            for (var key in this._scheme) obj[key] = obj[key] || this._scheme[key];
            if (this._scheme) {
                if (this._scheme.$init) this._scheme.$update(obj);
                else if (this._scheme.$update) this._scheme.$update(obj);
            }
        }
        ...
        if (dhtmlx.isNotDefined(index) || index < 0) index = data_size;
    ...
    idByIndex: function(index) {
        if (index >= this.order.length || index < 0) dhtmlx.log("Warning", "DataStore::idByIndex Incorrect index");
    ...
    */
    provideApi: function(target, eventable) {
        ...
        for (var i = 0; i < list.length; i++) target[list[i]] = dhtmlx.methodPush(this, list[i]);
        if (dhtmlx.assert_enabled()) this.assert_event(target);
    },
    /*
    ...
    */
    serialize: function() {
        var ids = this.order;
        var result = [];
        for (var i = 0; i < ids.length; i++) result.push(this.pull[ids[i]]);
        return result;
    }
};
dhtmlx.sort = {
    ...
    by: function(prop, method) {
        ...
        if (typeof method != "function") method = dhtmlx.sort.as[method || "string"];
...
dhtmlx.MouseEvents = {
    _init: function() {
        ...
        if (this.on_dblclick) dhtmlx.event(this._obj, "dblclick", this._onDblClick, this);
    ...
    _onContext: function(e) {
        ...
        if (id && !this.callEvent("onBeforeContextMenu", [id, e])) return dhtmlx.html.preventEvent(e);
    },
    /*
    ...
    _mouseEvent: function(e, hash, name) {
        ...
        while (trg && trg.parentNode) {
            if (!found && trg.getAttribute) { //if element with ID mark is not detected yet
                ...
                if (id) {
                    if (trg.getAttribute("userdata")) this.callEvent("onLocateData", [id, trg, e]);
            ...
            if (css) { //check if pre-defined reaction for element's css name exists
                ...
                if (hash[css]) return hash[css].call(this, e, id || dhtmlx.html.locate(e, this._id), trg);
            }
            trg = trg.parentNode;
        }
...
dhtmlx.Settings = {
    ...
    _parseSettings: function(obj, initial) {
        ...
        if (typeof obj == "object" && !obj.tagName) dhtmlx.extend(settings, obj);
    ...
    _parseContainer: function(obj, name, fallback) {
        ...
        */
        if (typeof obj == "object" && !obj.tagName) obj = obj.container;
        this._obj = this.$view = dhtmlx.toNode(obj);
        if (!this._obj && fallback) this._obj = fallback(obj);
    ...
    _set_type: function(name) {
        ...
        if (typeof name == "object") return this.type_setter(name);
...
dhtmlx.Template = {
    ...
    fromHTML: function(str) {
        ...
        if (this._cache[str]) return this._cache[str];
        //supported idioms
        // {obj} => value
...
dhtmlx.Type = {
    ...
    */
    add: function(obj, data) {
        ...
        if (!obj.types && obj.prototype.types) obj = obj.prototype;
        //if (typeof data == "string")
        //	data = { template:data };
        if (dhtmlx.assert_enabled()) this.assert_event(data);
            ...
            if (data.indexOf("->") != -1) {
                ...
                switch (data[0]) {
                    ...
                    case "http": //load from external file
                        data = new dhtmlx.ajax().sync().get(data[1], {
...
dhtmlx.compat = function(name, obj) {
    ...
    if (dhtmlx.compat[name]) dhtmlx.compat[name](obj);
};
