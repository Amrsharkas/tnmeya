...
dataProcessor.prototype = {
    ...
    serialize: function(data, id) {
        ...
        if (typeof id != "undefined") return this.serialize_one(data, "");
            ...
            if (window.dhtmlx && dhtmlx.security_key) stack.push("dhx_security=" + dhtmlx.security_key);
            return stack.join("&");
        }
    },
    ...
    _applyPayload: function(url) {
        if (this._payload)
            for (var key in this._payload) url = url + (url.indexOf("?") === -1 ? "?" : "&") + this.escape(key) + "=" + this.escape(this._payload[key]);
        return url;
    },
    ...
    sendAllData: function() {
        ...
        for (var i = 0; i < this.updatedRows.length; i++)
            if (this.getState(this.updatedRows[i]) !== "deleted") valid &= this.checkBeforeUpdate(this.updatedRows[i]);
    ...
    init: function(anObj) {
        this.obj = anObj;
        if (anObj._dp_init) anObj._dp_init(this);
        if (this.connector_init) anObj._dataprocessor = this;
    },
