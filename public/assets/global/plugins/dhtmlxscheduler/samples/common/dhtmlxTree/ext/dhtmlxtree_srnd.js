...
dhtmlXTreeObject.prototype._renderState = function() {
    ...
    if (!this._idpull[this.rootId]._sready) this.prepareSR(this.rootId, true);
...
dhtmlXTreeObject.prototype._buildSRND = function(z, skipParsing) {
    if (z.parentObject) this._globalIdStorageFind(z.parentObject.id);
    if (!this._idpull[this.rootId]._sready) this.prepareSR(this.rootId, true);
    this._renderItemSRND(z, this._getIndex(z));
    if ((z.unParsed) && (!skipParsing)) this.reParse(z, 0);
    if (!z.prepareSR) this.prepareSR(z.id);
}
    ...
    */
    ...
    this._get_srnd_p_last = function(id, p, mask) {
        ...
        while (true) {
            ...
            if (i._sxml && this.findStrInXML(i._sxml.d, "text", mask)) this._globalIdStorageFind(i.id);
    ...
    this._get_next_srnd = function(p, skip) {
        ...
        if (curr) return curr;
        p.pop();
        if (!p.length) return this.htmlNode;
    ...
    this._findNodeByLabel = function(searchStr, direction, fromNode) {
        ...
        while (fromNode = (direction ? this._get_prev_srnd(p, searchStr) : this._get_next_srnd(p))) {
            if (fromNode.label) {
                ...
                if (fromNode._sxml) {
                    if (fromNode._sxml.get("text").search(searchStr) != -1) return fromNode;
                    if (this.findStrInXML(fromNode._sxml.d, "text", searchStr)) this._globalIdStorageFind(fromNode.id);
                }
            }
            ...
            if (direction && p.length == 1 && p[0][1] == 0) break;
        }
        return null;
    };
    ...
    this._HideShow = function(itemObject, mode) {
        ...
        if (!itemObject.span) this._buildSRND(itemObject);
