...
dhtmlXTreeObject.prototype.activatePaths = function(height) {
    ...
    this.attachEvent("onXLE", function(xml) {
        ...
        var stpull = {};
        for (var i = 0; i < starts.length; i++) stpull[starts[i].getAttribute("id")] = starts[i];
    ...
    else if (this._idpull[0].childsCount) this._halfHeight = Math.floor(this._idpull[0].childNodes[0].span.parentNode.offsetHeight / 2);
    if (!this._halfHeight) this._halfHeight = 9;
    this.activatePaths = function() {}
}
dhtmlXTreeObject.prototype._clearPath = function(obj) {
    for (var i = obj.path.length - 1; i > 0; i--) {
        var t = obj.path[i];
        if (t._html) t._html.parentNode.removeChild(t._html);
        t._html = null;
    }
}
...
dhtmlXTreeObject.prototype.deletePath = function(id) {
    ...
    if (z) {
        ...
        for (var i = 0; i < this._paths.length; i++)
            if (this._paths[i] == z) return this._paths.splice(i, 1);
    }
};
dhtmlXTreeObject.prototype.deleteAllPaths = function(id) {
    for (var i = this._paths.length - 1; i >= 0; i--) this.deletePath(this._paths[i].id);
};
