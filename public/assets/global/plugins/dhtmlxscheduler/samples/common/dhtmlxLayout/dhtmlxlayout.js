...
function dhtmlXLayoutObject(base, pattern, skin) {
    ...
    this._init = function(pattern) {
        ...
        if (this.conf.mode != "c") {
            ...
            if (mainInst.conf.sep_idx == null) mainInst.conf.sep_idx = 1;
            else mainInst.conf.sep_idx++;
        ...
        if (t.cells != null) {
            for (var a in t.cells) {
                if (t.cells[a].layout != null) {
                    ...
                    this.cdata[a].attachLayout({
                        pattern: t.cells[a].layout
                    });
    ...
    this._getWindowMinDimension = function(win) {
        ...
        var t = {
            w: Math.max(w, 200),
            h: Math.max(h, 140)
        }; // default window min width/height
        win = null;
        return t;
    }
...
dhtmlXLayoutObject.prototype.setSeparatorSize = function(index, size) {
    ...
    for (var a in this.cdata) {
        ...
        if (a == "a" && this.sep != null) {
            index.current++;
            if (index.index == index.current) return {
                sep: this.sep
            };
        }
    }
...
window.dhtmlXLayoutCell = function(id, layout) {
    ...
    if (this.conf.mode != "c") {
        ...
        t.onclick = function(e) {
            if (that.conf.collapsed) that.expand();
            else that.collapse();
        }
        t = null;
    }
...
dhtmlXLayoutCell.prototype.setWidth = function(w) {
    if (this.conf.mode == "v") {
        ...
        var k = this.layout;
        var nextCell = k.cdata[k.conf.nextCell[this._idd]];
        for (var a in p.cdata)
            if (p.cdata[a].dataObj == k) p.cdata[a].setWidth(w);
        p = k = null;
    }
};
dhtmlXLayoutCell.prototype.setHeight = function(h) {
    if (this.conf.mode == "h") {
        ...
        if (nextCell.conf.collapsed) {
            ...
            if (p != null) {
                for (var a in p.cdata)
                    if (p.cdata[a].dataObj == k) p.cdata[a].setHeight(h);
            }
            p = k = nextCell = null;
            return;
        }
...
dhtmlXCellObject.prototype.attachLayout = function(conf) {
    ...
    if (typeof(window.dhtmlXTabBarCell) == "function" && this instanceof window.dhtmlXTabBarCell) {
        if (this.conf.skin == "dhx_skyblue") obj._ofs = {
            t: -1,
            r: -1,
            b: -1,
            l: -1
        };
        if (this.conf.skin == "material") obj._ofs = {
            t: 8,
            r: 8,
            b: 8,
            l: 8
