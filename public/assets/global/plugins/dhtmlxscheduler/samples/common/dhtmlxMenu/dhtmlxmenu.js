...
function dhtmlXMenuObject(base, skin) {
    ...
    this.conf = {
        ...
        of_ah: {
            ...
            dhx_terrace: 27,
            material: 25
        }, // arrow height+oplygon top/bottom padding
        of_ih: {
            dhx_skyblue: 24,
            dhx_web: 24,
            dhx_terrace: 24,
    ...
    this._addSubItemToSelected = function(item, polygon) {
        ...
        if (t == true) {
            this.conf.selected_sub.push(new Array(item, polygon));
        }
        return t;
    }
    ...
    this._bodyContext = function(e) {
        ...
        while (testZone != null) {
            if (testZone.id != null)
                if (that.isContextZone(testZone.id)) toHide = false;
    ...
    this.unload = function() {
        ...
        if (this.conf.context)
            for (var a in this.conf.ctx_zones) this.removeContextZone(a);
        if (this.cont != null) {
            this.cont.className = "";
...
dhtmlXMenuObject.prototype._updateItemComplexState = function(id, state, over) {
    ...
    if (item.childNodes[0])
        if (String(item.childNodes[0].className).search("complex_arrow") === 0) arrowObj = item.childNodes[0];
    ...
    if (imgSrc.length > 0) {
        if (imgObj != null) {
            ...
            if (isTopLevel) {
                ...
                if (!this.conf.rtl && this.idPull[id].childNodes.length > 0) this.idPull[id].insertBefore(imgObj, this.idPull[id].childNodes[0]);
                else this.idPull[id].appendChild(imgObj);
            } else {
                if (this.conf.icons_css == true) {
...
dhtmlXMenuObject.prototype._renderToplevelItem = function(id, pos) {
    ...
    if ((this.itemPull[id]["imgen"] != "") || (this.itemPull[id]["imgdis"] != "")) {
        ...
        if (imgTop) {
            if (this.conf.icons_css == true) {
                ...
                if (m.childNodes.length > 0 && !this.conf.rtl) m.insertBefore(i, m.childNodes[0]);
                ...
                if (m.childNodes.length > 0 && !this.conf.rtl) m.insertBefore(img, m.childNodes[0]);
    ...
    if (pos != null) this.cont.insertBefore(m, this.cont.childNodes[pos]);
    else this.cont.appendChild(m);
    this.idPull[m.id] = m;
    // create submenues
...
dhtmlXMenuObject.prototype._renderSublevelItem = function(id, pos) {
    ...
    if (pos != null && polygon.tbd.childNodes[pos] != null) polygon.tbd.insertBefore(tr, polygon.tbd.childNodes[pos]);
    else polygon.tbd.appendChild(tr);
...
dhtmlXMenuObject.prototype._xmlToJson = function(xml, parentId) {
    ...
    if (parentId == null) {
        ...
        if (root == null || (root != null && root.length == 0)) return {
            items: []
        };
        root = root[0];
    } else {
...
dhtmlXMenuObject.prototype.removeItem = function(id, _isTId, _recCall) {
    ...
    // clear nested items
    for (var a in this.itemPull)
        if (this.itemPull[a]["parent"] == id) this.removeItem(a, true, true);
