...
(function() {
    ...
    dhx.assert_error = function(message) {
        ...
        if (dhx.message && typeof message == "string") dhx.message({
            type: "debug",
            text: message,
            expire: -1
        });
        if (dhx.debug !== false) eval("debugger;");
    };
    //entry point for analitic scripts
    dhx.assert_core_ready = function() {
        if (window.dhx_on_core_ready) dhx_on_core_ready();
    };
    /*
    	Common helpers
    */
    ...
    dhx.extend = function(base, source, force) {
        ...
        if (source.defaults) dhx.extend(base.defaults, source.defaults);
    ...
    dhx.copy = function(source) {
        ...
        if (arguments.length > 1) {
            ...
            source = arguments[1];
        } else var target = (dhx.isArray(source) ? [] : {});
        for (var method in source) {
    ...
    dhx.protoUI = function() {
        if (dhx.debug_proto) dhx.log("UI registered: " + arguments[0].name);
        var origins = arguments;
        var selfname = origins[0].name;
        var t = function(data) {
            if (!t) return dhx.ui[selfname].prototype;
            var origins = t._dhx_proto_wait;
            if (origins) {
                var params = [origins[0]];
                for (var i = 1; i < origins.length; i++) {
                    params[i] = origins[i];
                    if (params[i]._dhx_proto_wait) params[i] = params[i].call(dhx, params[i].name);
                    if (params[i].prototype && params[i].prototype.name) dhx.ui[params[i].prototype.name] = params[i];
                }
                dhx.ui[selfname] = dhx.proto.apply(dhx, params);
                if (t._dhx_type_wait)
                    for (var i = 0; i < t._dhx_type_wait.length; i++) dhx.Type(dhx.ui[selfname], t._dhx_type_wait[i]);
                t = origins = null;
            }
            if (this != dhx) return new dhx.ui[selfname](data);
            else return dhx.ui[selfname];
        };
        t._dhx_proto_wait = Array.prototype.slice.call(arguments, 0);
        return dhx.ui[selfname] = t;
    };
    dhx.proto = function() {
        if (dhx.debug_proto) dhx.log("Proto chain:" + arguments[0].name + "[" + arguments.length + "]");
        var origins = arguments;
        ...
        for (var i = origins.length - 1; i > 0; i--) {
            ...
            if (origins[i].defaults) {
                ...
                for (var def in defaults)
                    if (dhx.isUndefined(compilation.defaults[def])) compilation.defaults[def] = defaults[def];
            }
            if (origins[i].type && compilation.type) {
                for (var def in origins[i].type)
                    if (!compilation.type[def]) compilation.type[def] = origins[i].type[def];
            }
            for (var key in origins[i]) {
                if (!compilation[key]) compilation[key] = origins[i][key];
            }
        }
        if (has_constructor) construct.push(compilation.$init);
        compilation.$init = function() {
            for (var i = 0; i < construct.length; i++) construct[i].apply(this, arguments);
        };
    ...
    dhx.require = function(module, callback, master) {
        if (typeof module != "string") {
            ...
            if (!count) {
                ...
                for (var file in module) dhx.require(file, callback, master);
            } else {
                callback = function() {
                    if (count) {
                        count--;
                        dhx.require(module[module.length - count - 1], callback, master);
                    } else return callback_origin.apply(this, arguments);
                };
                callback();
            }
            return;
        ...
        if (dhx._modules[module] !== true) {
            if (module.substr(-4) == ".css") {
                ...
                if (callback) callback.call(master || window);
                return;
            }
            ...
            if (!callback) {
                ...
                if (!dhx._modules[module]) { //first call
                    ...
                    dhx.ajax(dhx.codebase + module, function(text) {
                        ...
                        dhx._modules[module] = true;
                        for (var i = 0; i < calls.length; i++) calls[i][0].call(calls[i][1] || window, !i); //first callback get true as parameter
                    });
    ...
    dhx.event = function(node, event, handler, master) {
        ...
        var id = dhx.uid();
        if (master) handler = dhx.bind(handler, master);
        ...
        else if (node.attachEvent) node.attachEvent("on" + event, handler);
    ...
    dhx.eventRemove = function(id) {
        ...
        if (ev[0].removeEventListener) ev[0].removeEventListener(ev[1], ev[2], false);
        else if (ev[0].detachEvent) ev[0].detachEvent("on" + ev[1], ev[2]);
    ...
    dhx.log = function(type, message, details) {
        ...
        if (window.console && console.log) {
            ...
            if (window.console[type]) window.console[type](message || "unknown error");
    ...
    dhx.EventSystem = {
        ...
        on_setter: function(config) {
            if (config) {
                for (var i in config) {
                    if (typeof config[i] == 'function') this.attachEvent(i, config[i]);
                }
            }
        },
        //trigger event
        callEvent: function(type, params) {
            ...
            if (this._evs_map[type] && !this._evs_map[type].callEvent(type, params)) return_value = false;
            return return_value;
        },
    ...
    // dhx.env.transition
    (function() {
        if (navigator.userAgent.indexOf("Mobile") != -1) dhx.env.mobile = true;
        if (dhx.env.mobile || navigator.userAgent.indexOf("iPad") != -1 || navigator.userAgent.indexOf("Android") != -1) dhx.env.touch = true;
        if (navigator.userAgent.indexOf('Opera') != -1) dhx.env.isOpera = true;
        else {
        ...
        if (navigator.userAgent.toLowerCase().indexOf("android") != -1) dhx.env.isAndroid = true;
        ...
        if (dhx.env.isFF) prefix = '-Moz-';
        if (dhx.env.isWebKit) prefix = '-webkit-';
        if (dhx.env.isIE) prefix = '-ms-';
    ...
    dhx.html = {
        ...
        denySelect: function() {
            if (!dhx._native_on_selectstart) dhx._native_on_selectstart = document.onselectstart;
        ...
        createCss: function(rule) {
            var text = "";
            for (var key in rule) text += key + ":" + rule[key] + ";";
        ...
        create: function(name, attrs, html) {
            ...
            for (var attr_name in attrs) node.setAttribute(attr_name, attrs[attr_name]);
        ...
        remove: function(node) {
            if (node instanceof Array)
                for (var i = 0; i < node.length; i++) this.remove(node[i]);
            else
        ...
        insertBefore: function(node, before, rescue) {
            ...
            else rescue.appendChild(node);
        },
        //return custom ID from html element 
        //will check all parents starting from event's target
        locate: function(e, id) {
            if (e.tagName) var trg = e;
            else {
                e = e || event;
        ...
        posRelative: function(ev) {
            ev = ev || event;
            if (!dhx.isUndefined(ev.offsetX)) return {
                x: ev.offsetX,
                y: ev.offsetY
            }; //ie, webkit
            else return {
    ...
    (function() {
        ...
        dhx.event(window, "load", function() {
            ...
            dhx.delay(function() {
                dhx._ready = true;
                for (var i = 0; i < dhx._ready_code.length; i++) dhx._ready_code[i].call();
                dhx._ready_code = [];
            });
        });
    })();
    ...
    */
        ...
        */
        dhx.Type = function(obj, data) {
            if (obj._dhx_proto_wait) {
                if (!obj._dhx_type_wait) obj._dhx_type_wait = [];
            ...
            var name = data.name;
            var type = obj.type;
            if (name) type = obj.types[name] = dhx.clone(data.baseType ? obj.types[data.baseType] : obj.type);
            for (var key in data) {
                if (key.indexOf("template") === 0) type[key] = dhx.Template(data[key]);
                else type[key] = data[key];
            }
            return name;
        };
    })();
    ...
    dhx.Settings = {
        ...
        define: function(property, value) {
            if (typeof property == "object") return this._parseSeetingColl(property);
        ...
        _parseSettings: function(obj, initial) {
            ...
            var settings = {};
            if (initial) settings = dhx.extend(settings, initial);
            //code below will copy all properties over default one
            if (typeof obj == "object" && !obj.tagName) dhx.extend(settings, obj, true);
            //call config for each setting
            this._parseSeetingColl(settings);
        },
        _mergeSettings: function(config, defaults) {
            for (var key in defaults) switch (typeof config[key]) {
                case "object":
    ...
    */
    ...
    dhx.ajax.prototype = {
        ...
        getXHR: function() {
            if (dhx.env.isIE) return new ActiveXObject("Microsoft.xmlHTTP");
        ...
        */
        send: function(url, params, call) {
            var x = this.getXHR();
            if (!dhx.isArray(call)) call = [call];
            //add extra params to the url
            if (typeof params == "object") {
                var t = [];
                for (var a in params) {
                    var value = params[a];
                    if (value === null || value === dhx.undefined) value = "";
            ...
            if (this.request === 'POST') x.setRequestHeader('Content-type', 'application/x-www-form-urlencoded');
                    ...
                    if (call && self) {
                        for (var i = 0; i < call.length; i++) //there can be multiple callbacks
                            if (call[i]) {
                                ...
                                if (x.status >= 400 || (!x.status && !x.responseText)) method = call[i].error;
                                if (method) method.call((self.master || self), x.responseText, x.responseXML, x);
                            }
                    }
    ...
    dhx.AtomDataLoader = {
        ...
        load: function(url, call) {
            ...
            if (typeof call == "string") { //second parameter can be a loading type or callback
                ...
                call = arguments[2];
            } else if (!this.data.driver) this.data.driver = dhx.DataDriver.json;
            ...
            if (call) {
                if (dhx.isArray(call)) callback.push.apply(callback, call);
                else callback.push(call);
            }
        ...
        _onLoad: function(text, xml, loader, key) {
            ...
            } else this._onLoadError(text, xml, loader);
        ...
        _check_data_feed: function(data) {
            ...
            dhx.ajax(url, function(text, xml, loader) {
                ...
                if (data) this.setValues(data.getDetails(data.getRecords()[0]));
                else this._onLoadError(text, xml, loader);
    ...
    */
    ...
    dhx.DataDriver.json = {
        ...
        getRecords: function(data) {
            if (data && !dhx.isArray(data)) return [data];
            return data;
        },
    ...
    dhx.DataDriver.jsarray = {
        ...
        getDetails: function(data) {
            var result = {};
            for (var i = 0; i < data.length; i++) result["data" + i] = data[i];
            return result;
        },
    ...
    dhx.DataDriver.csv = {
        ...
        stringToArray: function(data) {
            ...
            for (var i = 0; i < data.length; i++) data[i] = data[i].replace(/^[ \t\n\r]*(\"|)/g, "").replace(/(\"|)[ \t\n\r]*$/g, "");
            return data;
        },
    ...
    dhx.DataDriver.xml = {
        ...
        toObject: function(text, xml) {
            ...
            if (typeof text == "string") var data = this.fromString(text.replace(/^[\s]+/, ""));
            else data = text;
            if (this._isValidXML(data)) return data;
            return null;
        },
        ...
        getInfo: function(data) {
            ...
            if (config.length) config = this.assignTypes(this.tagToObject(config[0], {}));
            else config = null;
            return {
        ...
        xpath: function(xml, path) {
            if (window.XPathResult) { //FF, KHTML, Opera
                ...
                //IE
                if (test) return xml.selectNodes(path);
                else {
        ...
        assignTypes: function(obj) {
            for (var k in obj) {
                var test = obj[k];
                if (typeof test == "object") this.assignTypes(test);
                else if (typeof test == "string") {
                    if (test === "") continue;
                    if (test == "true") obj[k] = true;
                    else if (test == "false") obj[k] = false;
                    else if (test == test * 1) obj[k] = obj[k] * 1;
                }
            }
            return obj;
        },
        ...
        tagToObject: function(tag, z) {
            ...
            if (a && a.length) {
                for (var i = 0; i < a.length; i++) z[a[i].name] = a[i].value;
                flag = true;
            }
            //map subtags
            ...
            for (var i = 0; i < b.length; i++) {
                if (b[i].nodeType == 1) {
                    ...
                    if (typeof z[name] != "undefined") {
                        if (!dhx.isArray(z[name])) z[name] = [z[name]];
                        z[name].push(this.tagToObject(b[i], {}));
                    } else z[b[i].tagName] = this.tagToObject(b[i], {}); //sub-object for complex subtags
                    flag = true;
                }
            }
            if (!flag) return this.nodeValue(tag);
        ...
        nodeValue: function(node) {
            if (node.firstChild) return node.firstChild.data; //FIXME - long text nodes in FF not supported for now
            return "";
        },
    ...
    */
    dhx.DataLoader = dhx.proto({
        ...
        _feed_callback: function() {
            ...
            if (typeof temp == "object" && (temp[0] != last[0] || temp[1] != last[1])) this.data.feed.apply(this, temp); //load last ignored request
        },
        //loads data from external URL
        load: function(url, call) {
        ...
        _onLoad: function(text, xml, loader) {
            ...
            if (data) this.data._parse(data);
            else return this._onLoadError(text, xml, loader);
        ...
        dataFeed_setter: function(value) {
            this.data.attachEvent("onBeforeFilter", dhx.bind(function(text, value) {
                if (this._settings.dataFeed) {
                    ...
                    var url = this._settings.dataFeed;
                    var urldata = [];
                    if (typeof url == "function") return url.call(this, value, filter);
                    for (var key in filter) urldata.push("dhx_filter[" + key + "]=" + encodeURIComponent(filter[key]));
        ...
        _call_onready: function() {
            if (this._settings.ready && !this._ready_was_used) {
                ...
                if (code) dhx.delay(code, this, arguments);
                this._ready_was_used = true;
            }
        },
        _call_onclearall: function() {
            for (var i = 0; i < this._ajax_queue.length; i++) this._ajax_queue[i].abort();
            this._ajax_queue = dhx.toArray();
        },
    ...
    */
    ...
    dhx.DataStore.prototype = {
        ...
        _parse: function(data, master) {
            ...
            if (this._filter_order) this.filter();
            //get size and position of data
            var info = this.driver.getInfo(data);
            if (info._key) dhx.securityKey = info._key;
            if (info._config) this.callEvent("onServerConfig", [info._config]);
            //get array of records
            var recs = this.driver.getRecords(data);
            this._inner_parse(info, recs);
            //in case of tree store we may want to group data
            if (this._scheme_group && this._group_processing) this._group_processing(this._scheme_group);
        ...
        _inner_parse: function(info, recs) {
            ...
            if (from === 0 && this.order[0]) { //update mode
                if (this._removeMissed) {
                    ...
                    marks = {};
                    for (var i = 0; i < this.order.length; i++) marks[this.order[i]] = true;
                }
                subload = false;
            ...
            for (var i = 0; i < recs.length; i++) {
                ...
                if (this.pull[id]) {
                    ...
                    if (this._scheme_update) this._scheme_update(this.pull[id]);
            ...
            if (marks) {
                this.blockEvent();
                for (var delid in marks) this.remove(delid);
                this.unblockEvent();
            }
            if (!this.order[info._size - 1]) this.order[info._size - 1] = dhx.undefined;
        },
        ...
        refresh: function(id) {
            ...
            if (id) this.callEvent("onStoreUpdated", [id, this.pull[id], "paint"]);
        ...
        getRange: function(from, to) {
            ...
            if (from) from = this.indexById(from);
            else from = (this.$min || this.startOffset) || 0;
            if (to) to = this.indexById(to);
            else {
        ...
        getIndexRange: function(from, to) {
            ...
            for (var i = (from || 0); i <= to; i++) ret.push(this.item(this.order[i]));
            return ret;
        },
        ...
        sync: function(source, filter, silent) {
            ...
            if (source.name != "DataStore") {
                if (source.data && source.data.name == "DataStore") source = source.data;
                else this._backbone_source = true;
            }
            var sync_logic = dhx.bind(function(mode, record, data) {
                ...
                if (dhx.debug_bind) dhx.log("[sync:request] " + this.debug_sync_master.name + "@" + this.debug_sync_master._settings.id + " <= " + this.debug_bind_master.name + "@" + this.debug_bind_master._settings.id);
                this.callEvent("onSyncApply", []);
                if (!silent) this.refresh();
                else silent = false;
            }, this);
            if (this._backbone_source) source.bind('all', sync_logic);
            else this._sync_events = [
        ...
        add: function(obj, index) {
            ...
            if (this._scheme)
                for (var key in this._scheme)
                    if (dhx.isUndefined(obj[key])) obj[key] = this._scheme[key];
            ...
            if (dhx.isUndefined(index) || index < 0) index = data_size;
            ...
            if (this._filter_order) { //adding during filtering
                ...
                if (!index && this.order.length) original_index = 0;
        ...
        remove: function(id) {
            ...
            if (dhx.isArray(id)) {
                for (var i = 0; i < id.length; i++) this.remove(id[i]);
                return;
            }
        ...
        idByIndex: function(index) {
            if (index >= this.order.length || index < 0) dhx.log("Warning", "DataStore::idByIndex Incorrect index");
            return this.order[index];
        },
        //converts index to id
        indexById: function(id) {
            var res = this.order.find(id); //slower than idByIndex
            if (!this.pull[id]) dhx.log("Warning", "DataStore::indexById Non-existing ID: " + id);
            return res;
        },
        ...
        */
        ...
        _filter_core: function(filter, value, preserve) {
            ...
            for (var i = 0; i < this.order.length; i++) {
                var id = this.order[i];
                if (filter(this.item(id), value)) neworder.push(id);
            }
        ...
        */
        each: function(method, master) {
            for (var i = 0; i < this.order.length; i++) method.call((master || this), this.item(this.order[i]));
        },
        ...
        removeMark: function(id, mark, css) {
            var obj = this._marks[id];
            if (obj && obj[mark]) delete obj[mark];
            if (css) {
        ...
        */
        provideApi: function(target, eventable) {
            ...
            for (var i = 0; i < list.length; i++) target[list[i]] = this._methodPush(this, list[i]);
        },
        /*
        ...
        */
        ...
        _sort: {
            ...
            _by: function(prop, method) {
                ...
                if (typeof method != "function") method = this._as[method || "string"];
    ...
    dhx.BaseBind = {
        ...
        bind: function(target, rule, format) {
            if (typeof target == 'string') target = dhx.ui.get(target);
            if (target._initBindSource) target._initBindSource();
            if (this._initBindSource) this._initBindSource();
            if (!target.getBindData) dhx.extend(target, dhx.BindSource);
            if (!this._bind_ready) {
            ...
            if (dhx.debug_bind) dhx.log("[bind] " + this.name + "@" + this._settings.id + " <= " + target.name + "@" + target._settings.id);
            ...
            if (!this._settings.dataFeed && this.loadNext) this.data.attachEvent("onStoreLoad", function() {
                target._bind_updated[target_id] = false;
            });
            if (this.isVisible(this._settings.id)) this.refresh();
        },
        ...
        _unbind: function(target) {
            ...
            if (events && target.data)
                for (var i = 0; i < events.length; i++) target.data.detachEvent(events[i]);
        }
    };
    //bind interface
    dhx.BindSource = {
        ...
        setBindData: function(data, key) {
            ...
            if (dhx.debug_bind) dhx.log("[bind:save] " + this.name + "@" + this._settings.id + " <= " + "@" + key);
            if (this.setValue) this.setValue(data);
            else if (this.setValues) this.setValues(data);
            else {
        ...
        getBindData: function(key, update) {
            ...
            if (target.isVisible(target._settings.id)) {
                ...
                if (dhx.debug_bind) dhx.log("[bind:request] " + this.name + "@" + this._settings.id + " => " + target.name + "@" + target._settings.id);
        ...
        _bind_specific_rules: function(obj) {
            if (obj.filter) dhx.extend(this, dhx.CollectionBind);
            else if (obj.setValue) dhx.extend(this, dhx.ValueBind);
            else dhx.extend(this, dhx.RecordBind);
        },
        ...
        _bind_update_common: function(target, rule, data) {
            if (target.setValue) target.setValue(data ? data[rule] : data);
            else if (!target.filter) {
                if (!data && target.clear) target.clear();
                else {
                    if (target._check_data_feed(data)) target.setValues(dhx.clone(data));
                }
            } else {
    ...
    dhx.ValueBind = {
        ...
        _bind_update: function(target, rule, format) {
            ...
            if (target.setValue) target.setValue(data);
            else if (!target.filter) {
                var pod = {};
                pod[rule] = data;
    ...
    if (window.dhtmlx) dhtmlx.BaseBind = dhx.BaseBind;
    dhtmlXDataStore = function(config) {
        ...
        obj[name] = function(dp) {
            ...
            this.data.attachEvent("onStoreUpdated", function(id, data, mode) {
                if (id && !dp._silent) dp.setUpdated(id, true, this._old_names[mode]);
            });
            varname = "_getRowData";
    ...
    if (window.dhtmlXDataView) dhtmlXDataView.prototype._initBindSource = function() {
        ...
        if (!this._settings.id) this._settings.id = dhx.uid();
        this.unbind = dhx.BaseBind.unbind;
        this.unsync = dhx.BaseBind.unsync;
        dhx.ui.views[this._settings.id] = this;
    };
    if (window.dhtmlXChart) dhtmlXChart.prototype._initBindSource = function() {
        this.isVisible = function() {
    ...
    dhx.BaseBind.legacySync = function(source, rule) {
        ...
        this.save = function(id) {
            ...
            for (var key in sobj)
                if (key.indexOf("$") !== 0) tobj[key] = sobj[key];
            source.refresh(id);
        };
        if (source && source.name == "DataCollection") return source.data.sync.apply(this.data, arguments);
        else return this.data.sync.apply(this.data, arguments);
    };
    if (window.dhtmlXForm) {
        ...
        dhtmlXForm.prototype.dataFeed = function(value) {
            if (this._settings) this._settings.dataFeed = value;
            else this._server_feed = value;
        };
            ...
            scheduler.sync = function(source, rule) {
                ...
                var insync = function(ignore) {
                    ...
                    for (var i = 0; i < order.length; i++) {
                        if (rule && rule.copy) evs[i] = dhx.clone(pull[order[i]]);
                        else evs[i] = pull[order[i]];
                    }
                ...
                this.attachEvent("onEventAdded", function(id, data) {
                    if (!source.data.pull[id]) source.add(data);
                });
                this.attachEvent("onEventDeleted", function(id) {
                    if (source.data.pull[id]) source.remove(id);
                });
                insync();
            };
    ...
    if (window.dhtmlXGridObject) {
        ...
        dhtmlXGridObject.prototype.sync = function(source, rule) {
            ...
            var insync = function(ignore) {
                ...
                if (count) {
                    ...
                    if (grid.pagingOn) grid.changePage();
                    else {
                        if (grid._srnd && grid._fillers) grid._update_srnd_view();
                        else {
                            grid.render_dataset();
                ...
                if (cursor && grid.setCursor) grid.setCursor(grid.rowsAr[cursor] ? cursor : null);
            ...
            grid.attachEvent("onEditCell", function(stage, id, ind, value, oldvalue) {
                if (stage == 2 && value != oldvalue) this.save(id);
                return true;
            });
            ...
            if (rule && rule.sort) grid.attachEvent("onBeforeSorting", function(ind, type, dir) {
            ...
            if (rule && rule.filter) {
                grid.attachEvent("onFilterStart", function(cols, values) {
                    ...
                    if (grid[name] && grid[name].length) return false;
            ...
            if (rule && rule.select) grid.attachEvent("onRowSelect", function(id) {
                source.setCursor(id);
            });
        ...
        dhtmlXGridObject.prototype._initBindSource = function() {
            if (dhx.isUndefined(this._settings)) {
                ...
                for (var i = 0; i < this[name]; i++)
                    if (!this.columnIds[i]) this.columnIds[i] = "cell" + i;
                ...
                this.attachEvent("onEditCell", function(stage, rId) {
                    if (stage === 2 && this.getCursor) {
                        if (rId && rId == this.getCursor()) this._update_binds();
                    }
                    return true;
                });
                this.attachEvent("onXLE", function() {
        ...
        dhtmlXGridObject.prototype.update = function(id, data) {
            for (var i = 0; i < this.columnIds.length; i++) {
                ...
                if (!dhx.isUndefined(data[key])) this.cells(id, i).setValue(data[key]);
            }
            var name = "_attrs";
            var attrs = this.getRowById(id)[name];
            for (var key in data) attrs[key] = data[key];
        ...
        dhtmlXGridObject.prototype.filter = function(callback, master) {
            ...
            if (this._settings.dataFeed) {
                ...
                } else if (dhx.isUndefined(callback)) filter = master;
                else filter[callback] = master;
                this.clearAll();
                var url = this._settings.dataFeed;
                if (typeof url == "function") return url.call(this, master, filter);
                var urldata = [];
                for (var key in filter) urldata.push("dhx_filter[" + key + "]=" + encodeURIComponent(filter[key]));
    ...
    if (window.dhtmlXTreeObject) {
        ...
        dhtmlXTreeObject.prototype.update = function(id, data) {
            if (!dhx.isUndefined(data.text)) this.setItemText(id, data.text);
        };
    }
    /*jsl:end*/
})();