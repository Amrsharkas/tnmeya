...
$(document).on('ready pjax:success', function() {
    $(document).on('change', '.master_dependency', function() { //more helpful in dynamically created elements
        ...
        if ($dependent_element.length == 0) {
            ...
            $dependent_element = $('*[data-depending-on="' + element_id + '"]');
        }
    ...
    $(document).on('change', ".ss-options-dependent", function() {
        ...
        $.each(menus_selectors, function(index) {
            ...
            var view = 'common_views.common_includes.dependent_options'; // Default view to render in the server
...
function retrieveChildren($selector = "body") {
    $($selector).find(".retrieve-children").each(function() {
        ...
        var toAppend = $(this).data('target'); //"#objectives.child.ol";
        var is_last_child = $(this).data('last-child'); //"#objectives.child.ol";
        retrieveView(retrieve_route, toAppend, is_last_child);
    });
}

function retrieveView(retrieve_route, toAppend, is_last_child) {
    appBlockUI();
    var array = toAppend.split("-");
    var ToClass = array[0];
    $.ajax({
...
$(document).on('click', '.removefile', function(e) {
    ...
    $.ajax({
        ...
        success: function() {
            ...
            if (type == "uploader_files") {
                $container.remove();
            } else {
                $container.find('.removefile').addClass('hidden');
            }
            $container.find('.filepicker').removeClass('hidden');
            $container.find('.hidden_uploader').removeClass('hidden');
...
function validateDependentValue($element, $depending_on_element_val, depending_on_value) {
    var $element_section_behavior = $element.attr('data-section-behavior');
    ...
    if (!depending_on_value.includes($depending_on_element_val)) { //In Array Dependency
        ...
        if ($element_section_behavior == "hide_section_only") {
            $element.hide(250);
        } else {
            $element.find("input,select").hide();
            $element.hide(250);
        }
        if ($element_type == "select_menu") {
            $element.find('.select_menu').val('');
        } else if ($element_type == "select_option") {
            $element.closest('.select_menu').val('');
        }
    } else {
...
function initValidateDependentValue($form) {
    //Hidden elements Dependencies
    $dependent_emements = $form.find(".dependent");
    ...
    $dependent_emements.each(function() {
        ...
        $depending_on_element = $("[id=" + depending_on_name + "]");
...
function initAjaxForm($form, $form_validation = null) {
    initValidateDependentValue($form);
    ...
    $call_function_with_name = $form.data('call-function');
    if (typeof window[$call_function_with_name] !== "undefined") {
        // safe to use the function
        window[$call_function_with_name]();
    }
    ...
    initTinyMces("#" + $form.attr("id"));
    // initTinyMceRichEditors($form.attr("id"));
}
