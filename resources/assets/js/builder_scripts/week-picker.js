(function() {
    ...
    WeekPicker.prototype = {
        ...
        init: function() {
            ...
            this.$picker = $('<div>').click(function(e) {
                ...
                .mousedown(function(e) {
                    e.preventDefault();
                }).addClass('weekpicker').append($nav, $calendar).insertAfter(this.$el);
            this.$el.focus(this.show).click(this.show).change($.proxy(function() {
                this.selectWeek();
            }, this));
            this.selectWeek();
            this.hide();
        ...
        rangeStart: function(date) {
            return this.findClosest(this.week_start, new Date(date.getFullYear(), date.getMonth()), -1);
        },
        rangeEnd: function(date) {
            return this.findClosest((this.week_start - 1) % 7, new Date(date.getFullYear(), date.getMonth() + 1, 0), 1);
        },
        update: function(s) {
            this.$target.val(s);
        ...
        nav: function(months) {
            var $subnav = $('<div>' + '<span class="prev button">&larr; Previous </span> | ' + ' <span class="next button">Next &rarr;</span>' + '</div>');
